...
`DEPENDENCIES.md` is a human-readable license inventory (from `pip-licenses`). `sbom.json` is a CycloneDX SBOM generated by the compliance script (`scripts/run-compliance-checks.sh`) and at release time via `.github/workflows/publish.yml`.
